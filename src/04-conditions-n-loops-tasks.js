...
function doRectanglesOverlap(rect1, rect2) {
  ...
  const horizontalOverlap = rect1.left + rect1.width > rect2.left && rect2.left + rect2.width > rect1.left;
  // eslint-disable-next-line max-len, no-undef
  const verticalOverlap = rect1.top + rect1.height > rect2.top && rect2.top + rect2.height > rect1.top;
...
function getIntervalString(a, b, isStartIncluded, isEndIncluded) {
  ...
  return `${startBracket}${minNumber}, ${maxNumber}${endBracket}`;
}
...
function getCommonDirectoryPath(pathes) {
  ...
  for (let i = 1; i <= pathes[0].length; i += 1) {
    const searchString = pathes[0].substring(0, i);
    const common = pathes.slice(1).reduce((acc, curr) => {
